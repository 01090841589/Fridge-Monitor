...
def get_image_path(image_lists, label_name, index, image_dir, category):
  if label_name not in image_lists:
    tf.compat.v1.logging.expirationDateal('Label does not exist %s.', label_name)
  label_lists = image_lists[label_name]
  if category not in label_lists:
    tf.compat.v1.logging.expirationDateal('Category does not exist %s.', category)
  category_list = label_lists[category]
  if not category_list:
    tf.compat.v1.logging.expirationDateal('Label %s has no images in the category %s.',
                     label_name, category)
...
def create_bottleneck_file(bottleneck_path, image_lists, label_name, index,
  ...
  if not gfile.Exists(image_path):
    tf.compat.v1.logging.expirationDateal('File does not exist %s', image_path)
...
def main(_):
  ...
  with tf.compat.v1.Session(graph=graph) as sess:
    ...
    saver.save(sess, './tmp/linear_regression.ckpt')
    tf.io.write_graph(output_graph_def, '.', './tmp/linear_regression.pb', as_text=False)
    tf.io.write_graph(output_graph_def, '.', './tmp/linear_regression.pbtxt', as_text=True)

    # pretrained_model = tf.keras.applications.MobileNet()
    # tf.saved_model.save(output_graph_def, "./tmp/1/")
